...
from aie.iron import ObjectFifo, Program, Runtime, Worker, Kernel, ExternalFunction
...
def vector_vector_add(input0, input1, output):
    ...
    def core_body(of_in1, of_in2, of_out, vec_kernel):
        # Number of sub-vector "tile" iterations
        for _ in range_(size):
            elem_in1 = of_in1.acquire(1)
            elem_in2 = of_in2.acquire(1)
            elem_out = of_out.acquire(1)
            vec_kernel(elem_in1, elem_in2, elem_out, n)
            of_in1.release(1)
            of_in2.release(1)
            of_out.release(1)

    # Create a worker to run the task on a compute tile
    worker = Worker(core_body, fn_args=[of_in1.cons(), of_in2.cons(), of_out.prod(), vec_add_kernel])
