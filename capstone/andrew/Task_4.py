...
def transform_with_internal_func_from_file(input1, input2, output):
    ...
    a_dims = None
    memA = inA.cons().forward(placement=Tile(0,1), name="memA", dims_to_stream=a_dims)

    # Input B
    inB = ObjectFifo(data_ty, name="inB")
    b_dims = None
    memB = inB.cons().forward(placement=Tile(0,1), name="memB", dims_to_stream=b_dims)

    # Output C
    memC = ObjectFifo(data_ty, name="memC")
    c_dims = None
    outC = memC.cons().forward(placement=Tile(0,1), name="outC", dims_to_stream=c_dims)
